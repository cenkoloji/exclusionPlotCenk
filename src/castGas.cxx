...
castGas::castGas(double atw, castMagnet *CASTM){
atomicwg = atw;
type=0;
mag = CASTM;
}*/ // }}}

/* Obsolete in Juanans code. Will see later {{{
castGas::castGas(double atw, castMagnet *CASTM, char *fileIn){
atomicwg = atw;
type=3;
mag = CASTM;
entries=0;
...
FILE *fIn = fopen(fileIn,"r");
	...
	fscanf(fIn,"%lf\t%lf\n",&pressmGamma[0][entries],&pressmGamma[1][entries]);
	entries++;
	}
...
if(type==0)return pressure*atomicwg/(mag->getTMag()*83144.72);
else if(type==1){double den = 1E-3*(0.02099*pressure-0.004914);if(den>=0.)return den;else return 0;}
else if(type==2){double den = 1E-3*(2.87E-5*pressure*pressure+0.02005*pressure-0.001571);if(den>=0.)return den;else return 0;}
else if(type==3)return pressmGamma[1][(int)((pressure-10.)*100.)]*pressmGamma[1][(int)((pressure-10.)*100.)]/548.810273;
...
if(type==0)return mag->getLenMag();
else if(type==1)return (815.7-2.694*pressure)*1E-2;
else if (type==2)return 7.82;
else if (type==3)return 7.26;
else return 0;
