...
for P in np.linspace(3000,12000,10):
    ...
    for i in range(0,len(fields)):
        ...
        pl.plot(ma,data[fields[i]],markers[i],mfc='none',mec=colors[i],markersize=16,label = label)
    ...
    for i in range(0,len(fields)-1):
        if fields[i] == "0":
            ...
            label = fields[i] + " - " + fields[-1]
        pl.plot(ma,data[fields[i]] - data[fields[-1]],markers[i],mfc='none',mec=colors[i],label = label)
